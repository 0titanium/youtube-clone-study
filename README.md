...
- (display videos, side videos, upload videos, display a video), {delete videos, check my upload videos} 


- (add to user's subscriptions, display subscriptions, display subscribers number, cancel subscriptions)
...
- 끝인줄 알았지만 좋아요 싫어요를 누르고 새로고침하면 계속 누를 수 있고, 좋아요 싫어요 수도 증가하는 것을 발견했다.


-> 새로고침할때마다 어떤 데이터가 변경되기 때문이 아닐까 싶다. 좋아요 싫어요를 눌렀는지에 대한 데이터가 초기화 된다.


-> redux로 어떤 작업을 해주던가 해야할 것 같다. 아니면


-> 원래 자신의 비디오나 댓글에는 좋아요 싫어요를 못누르게 하려고 어떤 처리를 했었는데 그게 문제였다. 생각해보니 자신의 비디오나


-> 댓글에 좋아요 싫어요를 누를 수 있어도 상관없지 않을까?


- 어제는 대댓글을 달면 자동으로 view가 켜졌는데 오늘은 또 안켜진다. 
...
- 댓글 삭제 버튼, 삭제기능 구현. 리렌더링 효과 필요.


-> CommentLists를 업데이트 할 때 배열에 추가하지 않고 서버에 댓글을 다시 요청하는 방식으로 처리했다.


-> 이전에는 댓글의 댓글을 달면 클릭해야 확인할 수 있었으나 이 방식으로 처리하니 바로 보이는 현상이 발견되었다.


-> 그런데 원래 댓글을 달면 바로 확인할 수 있는 것이 사용자 경험 측면에서 더 좋아보이기 때문에 놔두는 것이 좋을 것 같다.


- youtubu-clone-study 마무리.
